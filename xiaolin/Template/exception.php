...
<div class="xiaolin_info xiaolin_exception">
	<div class="xiaolin_info_img xiaolin_info_img_exception"></div>
	<div class="xiaolin_exception_errorInfo">
		<h2><?=l('异常捕获 _(:з」∠)_'); ?></h2>
		<div class="table-responsive">
			<table class="xiaolin_exception_errorInfo_errorText">
				<tr>
					<td><?=l('页面地址：'); ?></td>
					<td><?=safe_html($url); ?></td>
				</tr>
	...
	<?php if($debug){ ?>
	<div class="xiaolin_exception_vars">
		<h2><?=l('数据输出 (〜￣△￣)〜'); ?></h2>
	...
	<div class="xiaolin_exception_trace">
	...
	<?php } ?>
	<div class="xiaolin_exception_adminInfo">
...
$(function(){
	...
	$('.xiaolin_exception_trace>.item>.filepath>p:nth-child(1)').click(function(){
		$('.table-responsive', $(this).parent().parent()).toggle();
	});
	
	$('.allOpen').click(function(){
		if($(this).html() == '<?=l("[全部打开]"); ?>'){
			$('.xiaolin_exception_trace>.item .table-responsive').show();
			$(this).html('<?=l("[全部关闭]"); ?>');
		}else{
			$('.xiaolin_exception_trace>.item .table-responsive').hide();
			$(this).html('<?=l("[全部打开]"); ?>');
		}
	});
	
	$('.xiaolin_exception_vars table tr.sub').hide();
	$('.xiaolin_exception_vars td[colspan=2]').click(function(){

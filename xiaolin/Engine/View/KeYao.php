...
namespace XiaoLin\Engine\View;

use XiaoLin\XiaoLin;
use XiaoLin\Hook;
use XiaoLin\Exception\ViewException;
use XiaoLin\Engine\View\KeYao\Methods;
use XiaoLin\Engine\View\KeYao\Layout;
...
class KeYao {
	...
	static public function setIfMethod(string $name, $func){
		...
		self::$methods[$name] = function($match)use($name){
			...
			return '<?php if(\XiaoLin\Engine\View\KeYao::$customIfMethods[\'' . $name . '\'](' . $match[4] . ')): ?>';
		};
	...
	public function getFile($file, $isRoot){
		...
		if(!is_file($this->config['path'] . $cacheFile) || XiaoLin::$config['debug']){

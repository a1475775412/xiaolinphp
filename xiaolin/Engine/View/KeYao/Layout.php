...
namespace XiaoLin\Engine\View\KeYao;
...
class Layout {
	...
	static public function getMethods(){
		...
		$methods['section'] = function($match){
			...
			return '<?php \\XiaoLin\\Engine\\View\\KeYao\\Layout::setSection(' . substr($match[3], 1, -1) . ', function($__args){ extract($__args); foreach(\XiaoLin\View::$globalVar as $__globalVar){ global $$__globalVar; } unset($__args, $__globalVar); ?>';
		};
		
		$methods['push'] = $methods['prepend'] = function($match){
			if(!isset($match[4]) || $match[4] == '') return;
			
			return '<?php \\XiaoLin\\Engine\\View\\KeYao\\Layout::push(' . substr($match[3], 1, -1) . ', function($__args){ extract($__args); foreach(\XiaoLin\View::$globalVar as $__globalVar){ global $$__globalVar; } unset($__args, $__globalVar); ?>';
		};
		...
		$methods['yield'] = function($match){
			...
			return '<?php \\XiaoLin\\Engine\\View\\KeYao\\Layout::getSection(' . substr($match[3], 1, -1) . ', get_defined_vars()); ?>';
		};
		
		$methods['stack'] = function($match){
			if(!isset($match[4]) || $match[4] == '') return;
			
			return '<?php \\XiaoLin\\Engine\\View\\KeYao\\Layout::stack(' . substr($match[3], 1, -1) . ', get_defined_vars()); ?>';
		};
		
		$methods['include'] = function($match){
			if(!isset($match[4]) || $match[4] == '') return;
			
			return '<?php \\XiaoLin\\Engine\\View\\KeYao\\Layout::include(' . substr($match[3], 1, -1) . ', get_defined_vars()); ?>';
		};

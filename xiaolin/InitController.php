...
class InitController{
	...
	static private function returnHandle($return){
		...
		if(is_object($return) && get_class($return) === 'XiaoLin\\Model\\ControllerReturn'){
			switch($return->type){
				case 'status_success':
	...
	static private function error($type = 404){
		...
		switch($type){
			case 404:
				sysinfo(l('@xiaolin.sysinfo.statusCode.404', $type, [
					'code'		=> '%s',
			...
			case 500:
				sysinfo(l('@xiaolin.sysinfo.statusCode.500', $type, [
					'code'		=> '%s',
					'type'		=> 'error', //[info, error, success]
					'title'		=> '啊哈... 出了一点点小问题_(:з」∠)_',
					'info'		=> '页面找不到啦...',
			...
			case 403:
				sysinfo(l('@xiaolin.sysinfo.statusCode.403', $type, [
					'code'		=> '%s',
					'type'		=> 'error', //[info, error, success]
					'title'		=> '拒绝访问 (╯‵□′)╯︵┻━┻',
					'info'		=> '该页面拒绝访问',
